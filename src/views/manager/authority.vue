<template>
    <div class="customer">
        ...
        <ul class="rolelist">
            ...
            <Button type="ghost" @click="modal1 = true"><Icon type="plus-circled"></Icon>添加角色</Button>
            <Button type="ghost" @click="modal4 = true"><Icon type="minus-circled"></Icon>删除角色</Button>
    ...
    <div slot="footer">
        ...
        <div class="bomt">
            <ul class="aulist">
             ...
             <Button class="aubut" type="ghost" @click="modal2 = true"><Icon type="plus-circled"></Icon>添加权限</Button>
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidatedel: {
        menuId: [{ required: true, message: "请选择角色", trigger: "change" }]
      },
      ruleValidatepro: {
        roleName: [
          {
    ...
    Init: function() {
      ...
      api.axs("post", "/role/info", {}).then(({ data: { data, code } }) => {
        ...
        _this.roleid=_this.todos[0].id
         ...
         api
        .axs("post", "/menu/info", { parentId: "10" })
        .then(({ data: { data, code } }) => {
          _this.roles = data;
           _this.promisssid=_this.roles[0].id
          console.log( _this.promisssid);
              api
                .axs("post", "/menu/selectPermissionByMenuId", { menuId: _this.promisssid,roleId: _this.roleid})
                .then(({ data: { data, code } }) => {
