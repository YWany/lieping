<template>
    <div class="myPositions">
        <div class='currentNav'>当前位置: 我的职位
            <Button class='fr' @click="$router.push('/position/addNewPosition')">新增职位</Button>
        </div>
        <Tabs size="small" :animated=false @on-click="handleTabRemove">
            <TabPane label="我的职位" name="wdzw">
                <form class="searches">
                    <div class="search">
            ...
            <TabPane label="录用待入职" name="lydrz">
                <form class="searches">
                    <div class="search">
                        <Input v-model="form.searchVal" placeholder="请输入要搜索的内容...">
                        <Select v-model="form.selVal" slot="prepend" class='search-sels' style="width:80px">
            ...
            <TabPane label="入职未过保" name="ruwgb">
                <form class="searches">
                    <div class="search">
                        <Input v-model="form.searchVal" placeholder="请输入要搜索的内容...">
                        <Select v-model="form.selVal" slot="prepend" class='search-sels' style="width:80px">
...
export default {
    ...
    methods: {
        ...
        loadLists2(page) {
            ...
            this.form.pageNum = page;
            api
                .axs("post", "/job/queryMyRunningonJobs", this.form)
                .then(({ data }) => {
                    if (data.code === "SUCCESS") {
        ...
        loadLists3(page) {
            ...
            this.form.pageNum = page;
            api
                .axs("post", "/job/queryMyPendingPostJobs", this.form)
                .then(({ data }) => {
                    if (data.code === "SUCCESS") {
        ...
        handleTabRemove(name) {
            this.$store.state.spinShow = true;
            if (name == 'wdzw') this.loadLists();
            else if (name == 'jxz') this.loadLists2();
            else if (name == 'lydrz') this.loadLists3();
            else this.$store.state.spinShow = false;
        },
        seltime(data) {
        ...
        subSavedefe() {
            ...
            api.axs("post", "/job/jobPause", this.defeform).then(({ data }) => {
                if (data.code === "SUCCESS") {
        ...
        subSavetermination() {
            ...
            api
                .axs("post", "/job/jobStop", this.terminationform)
                .then(({ data }) => {
                    if (data.code === "SUCCESS") {
                        this.$Message.success(data.remark);
